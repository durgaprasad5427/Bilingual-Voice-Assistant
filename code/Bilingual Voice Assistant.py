...
def take_picture():
    try:
        ...
        return {"error": str(e)}
# Function to read Telugu text from a file
def read_telugu_text(file_path):
    with open(file_path, 'r', encoding='utf-8') as file:
        text = file.read()
    return text
def clear_existing_output(file_path):
...
def text_to_speech(text, file_prefix='output', language='te'):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"{file_prefix}_{timestamp}.mp3"
    clear_existing_output(output_file)
...
def handle_api_request(json_response):
    ...
    if module == "Weather":
        ...
        response = chat_session.send_message(parameters[0])
        gemres = response.text
        gemres = gemres.replace("*", "")

        # Translate Gemini response to Telugu
        translated_response = translator.translate(gemres, src='en', dest='te')
        telugu_response = translated_response.text

        # Speak the translated Telugu response
        audio_file = text_to_speech(telugu_response)
        print("సమాధానం...")
        play_speech(audio_file)
        engine.runAndWait()
        clear_existing_output(audio_file)
        return {"GeminiModule": telugu_response}
...
def main():
    ...
    engine.say("Hey, naa peru buji yemaina presnalu adagandi ")
    engine.runAndWait()
